   ...
   \begin{frame}\frametitle{Cумма векторов}
    ...
    \begin{frame}\frametitle{Скалярное произведение векторов}
    {
    	...
    	}
    	{
    		...
    		\only<2>{Если $\VV{a} \cdot \VV{b} = 0$, то $\VV a \mathbf ? \VV b$}% 
    		\only<3->{Если $\VV{a} \cdot \VV{b} = 0$, то $\VV a \mathbf \perp \vv b$}% 
    		
    		~
    		
    		\only<4>{Если $\ABS{\VV{a}},\ABS{\VV{b}} = 1$, то  $\VV{a} \cdot \VV{b} =$\only<5>{\dots}\pause {$\ \cos(\widehat{\VV{a} \, \VV{b}})$} 	}
...
\section{Вектора в пространстве}
...
\end{frame}
	...
	\begin{frame}\frametitle{Векторное произведение в движении}
	{
		...
		%\centering
		\animategraphics[autoplay,loop]{15}{Images/\jobname/vecpr - Copy}{0}{359} 		
...
\begin{frame}\frametitle{Прямая}
{
	...
	\TC{0.45}
	{
		...
		\MP{
			...
			\only<6->{\animategraphics[autoplay,loop, palindrome]{15}{Images/\jobname/line_anim}{0}{999} }
...
\begin{frame}\frametitle{Уравнение плоскости по точке и нормали}
{
	\TC{0.45}
	{
		\MP{
...
\begin{frame}\frametitle {Уравнение плоскости по трём точкам}
{
	\TC{0.45}
	{
...
\begin{frame}\frametitle {Задача: Находится ли точка внутри треугольника?(1)}
{
	\TC{0.35}
	{
...
\begin{frame}\frametitle {Задача: Находится ли точка внутри треугольника?(2)}
{
	\TC{0.55}
	{
		\centering%
		\only<1>{Пример 1\\ \includegraphics[page=2]{triangle2.pdf} }%
		\only<2>{Пример 1\\ \includegraphics[page=1]{triangle2.pdf} }%
		\only<3>{Пример 2\\ \includegraphics[page=4]{triangle2.pdf} }%
		\only<4>{Пример 2\\ \includegraphics[page=3]{triangle2.pdf} }%
	}
	{
		Метод 2 \\ ~ \\
...
\begin{frame}\frametitle {Задача: Находится ли точка внутри треугольника?(3)}
{
	\TC{0.45}
	{
