...
\everymath{\displaystyle}
	...
	\begin{frame}{Частные случаи}
		\TC{0.4}
		{
			\only<1>{\includegraphics[page=2]{digit_circles2.pdf}}%
			\only<2->{\includegraphics[page=3]{digit_circles2.pdf}}%
		}
		{
				\onslide<1->{%
	...
	\begin{frame}{Пространство вращения}
		...
		{%
			...
			Широта $A$ определяет угол вращения\\
			$\varphi=2\arccos w =$
	...
	\begin{frame}{Кватернионы}
		\TC{0.4}{
			\includegraphics[page=4]{digit_circles2.pdf}
	...
	\begin{frame}{Свойства кватернионов}
		...
		$h=a=(a,0)$ --- чисто скалярный (вещественный), $h\in\mathbb R$
...
\begin{frame}{Действия над кватернионами}
	...
	$hq=(a\alpha-\vv u\vv v,a\vv v+\alpha\vv u + \vv u \times \vv v)$
...
\end{frame}
	...
	\begin{frame}{Вернемся к вращению!}
		...
		Если кватернион $q =(a,\vv u)\ (|q|=1)$ описывает вращение, то (как в комплексных числах) $h'=qh$~---~есть вращение вектора $\vv h$ на угол вокруг оси, которые как-то заданы в $q$.
		
		~\pause
		
		$h'=qh = (a,\VV u)(0,\VV h) = (\underbrace{-\VV u \VV h}_{\neq 0}, a\vv h + \VV u \times \VV h)$ --- $h'$ <<улетел>>  в $\mathbb R^4$\pause
		
		Попробуем по другому:
		
		$h'=hq = (0,\VV h)(a,\VV u) = (\underbrace{-\VV h \VV u}_{\neq 0}, a\vv h + \VV h \times \VV u)$ --- $h'$ опять <<улетел>>  в $\mathbb R^4$
	...
	\begin{frame}{И наконец...}
		...
		\begin{block}{Вращение вектора $\vv h$ на угол $\varphi$ вокруг оси $\vv u$ }
		\centering
