...
\begin{document}
	...
	\pgfkeys{/pgf/number format/.cd,fixed,fixed zerofill, precision=2, print sign}	

	\foreach \frame in {0,4,...,359}
	{
	...
	\begin{tikzpicture}[scale=1, tdplot_main_coords]
		...
		\ROTATE{vecAC}{vecAC}{vpvecACvecAB}{\frame * 3.1415 / 180}
		...
		\draw [-{Latex[length=7pt]}, thick] (A) -- ($(A) + (O)!0.5!(vpvecACvecAB)$) node [left, midway] {$\displaystyle\frac{1}{2}\vv{c}$};
		
		\coordinate (D) at  ($(A) + (O)!0.5!(vpvecACvecAB)$); %вершина векторного произведения
		
		\VARPOINT{AA}{\Ax+\vpvecACvecABx*0.5}{\Ay+\vpvecACvecABy*0.5}{0}
		...
		\draw ($ (A)!10pt!(CC) $) -- +($(O)!10pt!(vpvecACvecAB)$) -- ($(A)!10pt!(D)$);
	     ...
	     $\vv{c} = \vv{a} \times \vv{b} $ \\
	     $\vv{a} = (
			\pgfmathprintnumber{\vecACx}, 
			\pgfmathprintnumber{\vecACy}, 
			\pgfmathprintnumber{\vecACz}
			)$\\
	     $\vv{b} = (
			\pgfmathprintnumber{\vecABx}, 
			\pgfmathprintnumber{\vecABy}, 
			\pgfmathprintnumber{\vecABz}
			)$ \\
	     $\vv{c} = (
			\pgfmathprintnumber{\vpvecACvecABx}, 
			\pgfmathprintnumber{\vpvecACvecABy}, 
			\pgfmathprintnumber{\vpvecACvecABz}
			)$ \\
