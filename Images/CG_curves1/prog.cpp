...
static std::string join_path(const fs::path &dir, const std::string &name)
{
    return (dir / name).string();
}

static void prepare_directory(const fs::path &dir)
{
    //  mkdir <dir> && del /q <dir>\*
...
int main()
{
    ...
    for(int k:{1,2,3,4,5})
    {
        prepare_directory(std::format("build_bspline_k{}_anim", k));
        {
    ...
    prepare_directory("build_bezier_anim");
    bc.p = {{0, 0}, {0.5, 2}, {3, 2.5}, {4, 0}};
    printTikzCoords("build_bezier_anim/tikz.dat", bc.p);
    curve.clear();
    ...
    for (auto &x : beziers)
    {
        ...
        prepare_directory(std::format("build_bezier_anim_n{}", n));
        bc.p = x;
        printTikzCoords(std::format("build_bezier_anim_n{}/tikz.dat",n), bc.p);
        curve.clear();
        bc.getAllCurve(100, curve);
        {
            std::vector<vector2d> cur_curve;
            ...
            {
                cur_curve.push_back(curve[fr]);
                print(std::format("build_bezier_anim_n{}/frame{}.dat",n,fr),  cur_curve);
                fout.open(std::format("build_bezier_anim_n{}/last_point_tikz_frame{}.dat",n, fr), std::ios::out);
    ...
    prepare_directory("build_bezier_p0_anim_n6");
    bc.p = {{0, 0}, {5, 4}, {6, 1}, {5, 0}, {-2, 3}, {7, 5}};
    printTikzCoords("build_bezier_p0_anim_n6/tikz.dat", bc.p);
    curve.clear();
    bc.getAllCurve(100, curve);
    {
        for (int fr = 0; fr <= 99; ++fr)
        {
            ...
            print(ss.str().data(), cur_curve);
            ss.str("");
            ss << "build_bezier_p0_anim_n6/tikz_frame" << fr << ".dat";
            printTikzCoords(ss.str().data(), bc.p);
        }
    }

    prepare_directory("build_bezier_p3_anim_n6");
    bc.p = {{0, 0}, {5, 4}, {6, 1}, {5, 0}, {-2, 3}, {7, 5}};
    printTikzCoords("build_bezier_p3_anim_n6/tikz.dat", bc.p);
    curve.clear();
    bc.getAllCurve(100, curve);
    {
        for (int fr = 0; fr <= 99; ++fr)
        {
            ...
            print(ss.str().data(), cur_curve);
            ss.str("");
            ss << "build_bezier_p3_anim_n6/tikz_frame" << fr << ".dat";
            printTikzCoords(ss.str().data(), bc.p);
        }
    }
    {
        prepare_directory("recursive_bezier_anim_n3");
        bc.p = {{0, 0}, {2, 4}, {3.1}};
        curve.clear();
        bc.getAllCurve(200,curve);
        printTikzCoords("recursive_bezier_anim_n3/tikz.dat", bc.p);
    ...
    {
        prepare_directory("recursive_bezier_anim_n4");
        bc.p = {{0, 0}, {0.5, 2}, {3, 2.5}, {4, 0}};
        curve.clear();
        bc.getAllCurve(200, curve);
        printTikzCoords("recursive_bezier_anim_n4/tikz.dat", bc.p);
