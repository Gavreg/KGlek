...
\begin{document}
	...
	\begin{tikzpicture}[ scale=1.2, tdplot_main_coords]
			...
			\clip[tdplot_screen_coords] (-5,-4) rectangle (10.5,7.5);
			...
			\draw [-Latex, blue] (0,-5.5,0) -- ($10*(j)$);	
			\draw [-Latex, green] (0,0,-1) -- ($8*(k)$) node [right] {$y$};
			
			\draw[xshift=10pt] (0,-5.5,0) node[below] {$-\dfrac{1}{a}$};
			...
			\foreach \p in {A,B,C,D}{
				\draw (\p) -- (\p 1);
				\node at (\p) {\p};
				\draw [ultra thin] (\p) -- (0,-5,0);
				\draw[ultra thin] (\p 1) -- (0,-5,0);
